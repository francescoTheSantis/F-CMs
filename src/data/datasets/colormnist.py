...
def update_concept_names_ColorMNIST(dataset, is_complex_coloring = False):
    """
...
def onehot_to_concepts_ColorMNIST(dataset, is_complex_coloring = False):
    """
    ...
    for split, data in dataset.data.items():
        ...
        colors = torch.argmax(data.c[:, 10:13], axis=1)
    ...
    def colorize_data(self):
        ...
        for index in range(len(self.data)):
            ...
            if self.coloring_mode == "complex":
                c_label = np.zeros(13 + len(self.possible_scales) + len(self.possible_degrees))  # 10 digits + 3 colors + scales + degrees
                c_label[13 + self.possible_scales.index(scale)] = 1
                c_label[13 + len(self.possible_scales) + self.possible_degrees.index(degree)] = 1
