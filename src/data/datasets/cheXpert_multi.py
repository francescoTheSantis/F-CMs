...
VAL_SPLIT_FILENAME = "custom_val_multimodal.csv"
TEST_SPLIT_FILENAME = "custom_test_multimodal.csv"
ALL_BALANCED_TRAIN_SPLIT_FILENAME = "custom_train_multimodal_all_balanced.csv"
ALL_BALANCED_VAL_SPLIT_FILENAME = "custom_val_multimodal_all_balanced.csv"
ALL_BALANCED_TEST_SPLIT_FILENAME = "custom_test_multimodal_all_balanced.csv"
...
def _class_counts(df: pd.DataFrame, target_name: str = TARGET_NAME) -> Dict[int, int]:
    return {
        int(label): int(count)
        for label, count in df[target_name].value_counts().sort_index().items()
    }
...
def _split_balanced_dataframe(
    ...
    balanced_df, _ = _balance_task_classes(df, target_name=target_name, seed=seed)

    split_parts = {"train": [], "val": [], "test": []}
    for label in sorted(balanced_df[target_name].dropna().unique()):
        label_df = _shuffle(balanced_df[balanced_df[target_name] == label], seed + int(label))
        n_label = len(label_df)
        train_end = int(n_label * train_fraction)
        val_end = train_end + int(n_label * val_fraction)

        split_parts["train"].append(label_df.iloc[:train_end])
        split_parts["val"].append(label_df.iloc[train_end:val_end])
        split_parts["test"].append(label_df.iloc[val_end:])

    split_dfs = {}
    for idx, split_name in enumerate(["train", "val", "test"]):
        if len(split_parts[split_name]) == 0:
            split_dfs[split_name] = pd.DataFrame(columns=df.columns)
        else:
            split_dfs[split_name] = _shuffle(
...
def build_multimodal_metadata(
    ...
    task_class_counts = _class_counts(merged_df, target_name=TARGET_NAME)
    stats = {
        "text_source": text_source,
        "rows_before_text_filter": int(rows_before_text_filter),
        "rows_after_text_filter": int(len(merged_df)),
        ...
        "task_balancing": {
            "applied": False,
            "reason": "split_level_balancing",
            "class_counts_before": task_class_counts,
            "class_counts_after": task_class_counts,
        },
    }
    with open(stats_path, "w") as handle:
        json.dump(stats, handle, indent=2)
...
def create_patient_splits(
    ...
    for split_name, patients in split_map.items():
        split_df = df[df["patient_id"].isin(patients)].copy()
        if balance_train_val and split_name in {"train", "val"}:
            split_df, _ = _balance_task_classes(
                split_df,
                target_name=TARGET_NAME,
                seed=seed + (0 if split_name == "train" else 1),
            )
        split_df = split_df[["sample_id"]].copy()
        split_df.to_csv(split_files[split_name], index=False)

    return split_files


def create_all_balanced_splits(
    metadata_path: Path,
    processed_root: Path = PROCESSED_MULTIMODAL_DIR,
    seed: int = 42,
) -> Dict[str, Path]:
    processed_root.mkdir(parents=True, exist_ok=True)
    df = pd.read_csv(metadata_path)
    split_dfs = _split_balanced_dataframe(df, seed=seed, target_name=TARGET_NAME)

    split_files = {
        "train": processed_root / ALL_BALANCED_TRAIN_SPLIT_FILENAME,
        "val": processed_root / ALL_BALANCED_VAL_SPLIT_FILENAME,
        "test": processed_root / ALL_BALANCED_TEST_SPLIT_FILENAME,
    }

    for split_name, split_df in split_dfs.items():
        split_df[["sample_id"]].to_csv(split_files[split_name], index=False)
...
class CheXpertMulti:
    ...
    def __init__(
        ...
        self.active_modality = active_modality
        self.text_source = text_source
        self.all_balanced = all_balanced
    ...
    def split(self, seed: Optional[int] = None, all_balanced: Optional[bool] = None):
        seed = 42 if seed is None else seed
        use_all_balanced = self.all_balanced if all_balanced is None else all_balanced
        self.all_balanced = use_all_balanced
        metadata_path = build_multimodal_metadata(
            raw_root=RAW_CHEXPERT_DIR,
            processed_root=PROCESSED_MULTIMODAL_DIR,
            text_source=self.text_source,
        ...
        if use_all_balanced:
            split_files = create_all_balanced_splits(
                metadata_path,
                PROCESSED_MULTIMODAL_DIR,
                seed=seed,
            )
        else:
            split_files = create_patient_splits(
                metadata_path,
                PROCESSED_MULTIMODAL_DIR,
                seed=seed,
                balance_train_val=True,
            )
