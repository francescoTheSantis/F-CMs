...
def main() -> None:
    ...
    for client_id in range(cfg.n_clients):
        ...
        # Evaluate on client
        loss_test, accuracy_test, f1_test = models.simple_test(model, device, test_loader)
        print(f"\033[93mClient {client_id} - Test Loss: {loss_test:.3f}, Test Accuracy: {accuracy_test*100:.2f}, F1-score: {f1_test*100:.2f}  \033[0m")
        accuracies.append(accuracy_test)
        losses.append(loss_test)
        f1.append(f1_test)
    
    # Averaged accuracy across clients   
    print(f"\n\033[93mAverage Test Loss: {np.mean(losses):.3f}, Average Test Accuracy: {np.mean(accuracies)*100:.2f}, Average F1-score: {np.mean(f1_test)*100:.2f}\033[0m\n")
